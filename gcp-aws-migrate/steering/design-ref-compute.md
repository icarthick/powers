...
## GKE → ECS Fargate Mandatory Resources

When mapping a GKE cluster (`google_container_cluster`) to ECS Fargate, **always** include ALL of the following in `new_aws_resources`:

| Resource                    | Always required? | Condition                                              |
| --------------------------- | ---------------- | ------------------------------------------------------ |
| `aws_ecs_cluster`           | Yes              | Unless already created by a prior cluster              |
| `aws_ecs_service`           | Yes              | One per node pool                                      |
| `aws_ecs_task_definition`   | Yes              | One per node pool                                      |
| `aws_ecr_repository`        | **Always**       | Images must be re-pushed from GCR/AR to ECR            |
| `aws_lb`                    | **Always**       | GKE has implicit ingress; ECS needs ALB                |
| `aws_lb_target_group`       | Yes, with ALB    | One per service family                                 |
| `aws_lb_listener`           | Yes, with ALB    | HTTP 80 and HTTPS 443                                  |
| `aws_appautoscaling_target` | When configured  | When node pool has `autoscaling` block                 |
| `aws_appautoscaling_policy` | When configured  | When `aws_appautoscaling_target` is added              |

**PROHIBITED omissions:**
- Never omit `aws_ecr_repository`. Container images cannot be pulled from GCR on ECS.
- Never omit `aws_lb` unless the cluster is explicitly documented as internal-only.
- Never omit autoscaling resources when node pool autoscaling is configured.

## google_container_node_pool Handling

**CRITICAL: `google_container_node_pool` is NOT eligible for Pass 1. Evaluate in Pass 2 AFTER its parent cluster is resolved.**

**When cluster → ECS Fargate or ECS EC2:**
- **Target: `absorbed`** — node pool parameters become ECS task definition and service scaling config.
- Carry forward: `machine_type` → vCPU/memory, `min_node_count` → min task count, `max_node_count` → max task count, `autoscaling` block → triggers `aws_appautoscaling_target` + `aws_appautoscaling_policy`.

**When cluster → EKS:**
- **Target: `aws_eks_node_group`** — one node pool → one EKS managed node group.
