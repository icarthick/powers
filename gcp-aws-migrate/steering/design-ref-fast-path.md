...
**Add-ons (ALB, NAT, etc.):** A row may say "Fargate" while the architecture diagram also includes an **ALB** or **NAT Gateway** from **other** Terraform resources. Confidence is still per **resource row** — e.g. `google_cloud_run_service` = `deterministic`; `google_compute_forwarding_rule` + backend = often `inferred` (see `networking.md`).
...
| `google_project_service`      | API enablement is implicit on AWS. Services are enabled by default.                   |
...
| `data.google_service_account` | Data source only (read-only). No mapping needed.                                      |
...
5. If not found: use `steering/design-ref-index.md` to determine category → apply rubric in that category's file
